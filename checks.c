...
int
hibachk_authorize(const struct hibaenv *env, const struct hibaext *grant, u_int32_t id, const char *role) {
	int ret;
	u_int32_t i;
	u_int32_t version;
	u_int32_t min_version;
	...
	if (env->grl != NULL && (ret = hibagrl_check(env->grl, env->cert_serial, id)) < 0) {
		return ret;
	}
