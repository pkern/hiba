...
get_logs() {
	...
	PGM='BEGIN{FS=","}{if ($1 >= cutoff) {print '$FIELDS'}}'
	now=$(date +%s)
	cutoff=$((now - 86400 * DAYS))
	cat "$dest/logs" | awk -v cutoff=$cutoff 'BEGIN{FS=","}{if ($1 >= cutoff) { print $1 " " $2}}'
}
...
cleanup_logs() {
	...
	COUNT=$((old - new))
	if [ "$COUNT" -gt 0 ]; then
		echo "== Do you want to removed $COUNT entries before $(date --date=@$cutoff)? (y|N)"
		read ans
		if [ "$ans" = "y" ]; then
...
revoke_grants() {
	...
	SERIALS=$(cat "$dest/logs" | awk -v NAME=$NAME -v GRANT=$GRANT 'BEGIN{FS=","}{ if(($3 ~ NAME) && ($4 ~ GRANT)) {print $2}}')
	...
	for s in $SERIALS; do
		GRANTS_BY_SERIAL=$(cat "$dest/logs" | awk -v serial=$s 'BEGIN{FS=","}{ if ($2 == serial) { print $4 }}')
...
sign() {
	...
	ssh-keygen -I "$ID" -s "$dest/ca" $T -z "$serial" -n "$PRINCIPALS" -V "$VALIDITY" "${HIBAOPTS[@]}" "$@" "$TARGET.pub"
	if [ $? -eq 0 ]; then
