...
struct hibagrl*
hibagrl_new() {
	struct hibagrl *grl = malloc(sizeof(struct hibagrl));
	return grl;
}
...
hibagrl_decode(struct hibagrl *grl, struct sshbuf *blob) {
	...
	if (log_level_get() >= SYSLOG_LEVEL_DEBUG3) {
		debug3("hibagrl_decode: serial list section content @%" PRIu64, blob->off);
	...
	if (log_level_get() >= SYSLOG_LEVEL_DEBUG3) {
		debug3("hibagrl_decode: bitmap section content @%" PRIu64, blob->off);
...
hibagrl_encode(const struct hibagrl *grl, struct sshbuf *blob) {
	...
	debug3("hibagrl_encode: serial list section @%" PRIu64, sshbuf_len(blob));
	...
	debug3("hibagrl_encode: bitmap section @%" PRIu64, sshbuf_len(blob));
	...
	while (r->next != NULL) {
		...
		offset += r->size + sizeof(u_int16_t);
...
#define _BLOCK_FOR_ID(id) ((id) >> 3)
#define _OFFSET_FOR_ID(id) ((id) & 0x7)

// We want to update the offset in the right block.
#define REVOKE_ID(r, id) do { \
		debug("REVOKE_ID(%d): map[%d] |= %02x", id, _BLOCK_FOR_ID(id), _OFFSET_FOR_ID(id)); \
		((r)->map[_BLOCK_FOR_ID(id)] |= 1 << _OFFSET_FOR_ID(id)); \
	} while (0)
...
hibagrl_revoke_grant(struct hibagrl *grl, u_int64_t serial, u_int32_t lo, u_int32_t hi) {
	...
	required_size = _BLOCK_FOR_ID(hi) + 1;
...
// - the map size is larger or equal to the grant ID.
// - the grant ID corresponding bit is set.
#define IS_REVOKED_ID(r, s, id) ((_BLOCK_FOR_ID(id) <= (s)) && \
	((1 << _OFFSET_FOR_ID(id)) & (r)[_BLOCK_FOR_ID(id)]))

int
hibagrl_check(const struct hibagrl *grl, u_int64_t serial, u_int32_t grant_id) {
	u_int64_t offset;
	u_int64_t size;
	u_int64_t min = 0;
	u_int64_t max;
	...
	if (IS_REVOKED_ID(grl->bitmaps + offset, size, grant_id))
